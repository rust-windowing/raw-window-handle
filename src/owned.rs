...
macro_rules! handle_type {
    (
        ...
        impl $name {
            ...
            #[doc = concat!("  - In this rule, the term \"", $type, "\" refers to")]
            ///      the underlying system resource.
            #[doc = concat!(" - The [`", stringify!($raw), "`] must be valid for")]
            ///   the lifetime of this handle.
            /// - `retain`ing a handle and then immediately `release`ing the handle
            ///   must have no side effects. The `get` function is allowed to be
            ...
            #[doc = concat!("Try to retain this [`", stringify!($name), "`] in a")]
            /// fallible way.
            ///
            /// In certain cases it's possible for increment the refcount of a
            #[doc = concat!("[`", stringify!($name), "`] to fail. While the")]
            /// `Clone` implementation for this type panics if an error occurs,
            /// this function allows one to deal with the error.
            // TODO: Add example.
            #[inline]
            pub fn try_clone(&self) -> Result<Self, HandleError> {
                unsafe {
                    ...
                    let new = (self.vtable.retain)(self.data)?;

                    // SAFETY: The `retain`ed pointer is known to be valid for
                    // the current vtable.
                    Ok(Self::from_raw_parts(new, self.vtable))
                }
            }
        ...
        impl Clone for $name {
            ...
            fn clone(&self) -> Self {
                self.try_clone()
                    .expect(concat!("failed to increment refcount of `", stringify!($name), "`"))
            }
        }
        ...
        impl<T: $as_borrowed + 'static> From<alloc::rc::Rc<T>> for $name {
            ...
            /// ```no_run
            /// use raw_window_handle::{
            #[doc = concat!("   ", stringify!($as_borrowed), ", ", stringify!($name), ", ", stringify!($raw))]
            /// };
            /// use std::rc::Rc;
            ///
            /// struct Foo;
            ...
            ///     #[inline]
            #[doc = concat!("    fn ", stringify!($as_borrowed_name), "(&self) -> Result<", stringify!($borrowed), "<'_>, HandleError> {")]
            ///         /* ... */
            /// # todo!()
            ///     }
            /// }
